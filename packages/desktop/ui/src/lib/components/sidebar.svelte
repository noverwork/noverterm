<script lang="ts">
  ...
  function localTerminalButtonClass(): string {
    if (activeSection === "terminal") {
      return "h-10 justify-start gap-2 rounded-2xl border-emerald-300/18 bg-emerald-300/8 text-emerald-100 shadow-[0_10px_26px_rgb(52_211_153/0.08)] ring-1 ring-emerald-300/8 hover:border-emerald-300/25 hover:bg-emerald-300/12 hover:text-white";
    }

    return "h-10 justify-start gap-2 rounded-2xl border-emerald-300/12 bg-emerald-300/[0.045] text-emerald-200/85 hover:border-emerald-300/22 hover:bg-emerald-300/8 hover:text-emerald-50";
  }
</script>

<aside
  class="sidebar relative flex flex-col border-r border-white/10 bg-[#070b12]/96 shadow-[18px_0_60px_rgb(0_0_0/0.28)] backdrop-blur-2xl transition-all duration-300 {collapsed
    ? 'w-[4.5rem]'
    : 'w-[20.5rem]'}"
>
  <div
  ...
  <div class="flex items-center justify-between px-4 py-4">
    {#if !collapsed}
      <div class="min-w-0">
        <div class="flex items-center gap-3">
          <div
            class="grid size-10 place-items-center rounded-2xl border border-cyan-300/18 bg-cyan-300/10 text-cyan-200 shadow-[0_0_24px_rgb(34_211_238/0.12)]"
          >
            <Terminal class="size-4" />
          </div>
          <div class="min-w-0">
            <p
              class="text-[10px] font-semibold uppercase tracking-[0.28em] text-cyan-200/70"
            >
              Noverterm
            </p>
    ...
    {:else}
      <div
        class="mx-auto grid size-11 place-items-center rounded-2xl border border-cyan-300/18 bg-cyan-300/10 text-cyan-200"
      >
        <Terminal class="size-4" />
      </div>
    {/if}
              ...
              {#if keyCount > 0}
                <span
                  class={activeSection === "keys"
                    ...
                    : "ml-auto rounded-full bg-cyan-300/12 px-1.5 py-0.5 text-[10px] text-cyan-200"}
                  >{keyCount}</span
                >
              {/if}
            </Button>
  ...
  {:else}
    <div class="flex flex-1 flex-col items-center gap-2 px-2 py-4">
      {#if onLocalTerminal}
        <Button
          variant="ghost"
          size="icon-sm"
          class={iconButtonClass("terminal")}
          onclick={onLocalTerminal}
      ...
      {#if onPortForwards}
        <Button
          variant="ghost"
          size="icon-sm"
          class={iconButtonClass("forwards")}
          onclick={onPortForwards}
    ...
    <div
      class="flex flex-col items-center gap-2 border-t border-white/10 px-2 py-4"
    >
      {#if onOpenSettings}
        <Button
          variant="ghost"
      ...
      {#if onLogout}
        <Button
          variant="ghost"
          size="icon-sm"
          class="rounded-2xl text-slate-500 hover:bg-red-400/10 hover:text-red-300"
          onclick={onLogout}
          aria-label="Log out"
          title="Log out"
        >
