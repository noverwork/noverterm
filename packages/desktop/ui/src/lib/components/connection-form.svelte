    ...
    >
      <div>
        ...
        <p class="mt-2 text-sm text-slate-500">
          Save host details and encrypted credentials for terminal and
            ...
            <div class="rounded-2xl border border-white/8 bg-white/[0.035] p-4">
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="mt-1 text-xs text-slate-400">
                    Name the host and define the SSH endpoint.
                  </p>
                </div>
              </div>
              ...
              <div class="mt-4 grid gap-4 sm:grid-cols-[minmax(0,1fr)_7rem]">
                ...
                <div class="space-y-2">
                  <label
                    for="conn-host"
                    class="text-sm font-medium text-slate-100">Hostname</label
                  >
                  <Input
                    id="conn-host"
                ...
                <div class="space-y-2">
                  <label
                    for="conn-port"
                    class="text-sm font-medium text-slate-100">Port</label
                  >
                  <Input
                    id="conn-port"
                    type="number"
                ...
                <div class="space-y-2 sm:col-span-2">
                  <label
                    for="conn-username"
                    class="text-sm font-medium text-slate-100">Username</label
                  >
                  <Input
                    id="conn-username"
                ...
                <div class="space-y-1">
                  ...
                  <p class="text-sm leading-6 text-slate-400">
                    {#if connection?.sshKeyId}
                      Select a saved key, paste a replacement, or disable key
                      auth.
                    {:else}
                  ...
                  {#if keys.length > 0}
                    <div class="space-y-2">
                      ...
                      {#if selectedKeyWillBeUsed}
                        <p class="text-xs text-emerald-400">
                          Using saved key: {keys.find(
                            (key) => key.id === $formData.selectedKeyId,
                          )?.name ?? ""}
                        </p>
                      {/if}
                    </div>
                    ...
                    <div class="flex items-center gap-3 text-xs text-slate-400">
                      ...
                      <button
                        ...
                        onclick={() =>
                          handleKeyModeChange(
                            $formData.keyMode === "new" ? "saved" : "new",
                          )}
                        disabled={isSaving}
                      >
                        {$formData.keyMode === "new"
                          ? "select a saved key instead"
                          : "paste a new key"}
                      </button>
                    </div>
                  {/if}
                  ...
                  {#if pastingNewKey || keys.length === 0}
                    ...
                    <div class="space-y-2">
                      <label
                        for="conn-passphrase"
                        class="text-sm font-medium text-slate-100"
                        >Key passphrase <span class="text-slate-500"
                          >(optional)</span
                        ></label
                      >
                      <Input
