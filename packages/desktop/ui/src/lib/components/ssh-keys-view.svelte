...
<div class="flex h-full flex-col overflow-y-auto bg-background px-5 py-6 lg:px-8">
  <section class="panel-glass flex min-h-full flex-col rounded-[1.75rem] p-5 text-white sm:p-6">
    <div class="flex flex-col gap-4 border-b border-white/10 pb-5 sm:flex-row sm:items-start sm:justify-between">
      ...
      {#if !showForm}
        <Button onclick={openNewKeyForm} variant="default" size="sm" class="gap-2 self-start rounded-xl">
          <Plus class="size-3.5" />
          Add key
        </Button>
      {/if}
    ...
    {#if showForm}
      <form class="mt-5 rounded-2xl border border-primary/30 bg-primary/5 p-5" onsubmit={handleSubmit}>
        <div class="flex items-center justify-between gap-3">
          <h2 class="text-sm font-semibold text-primary">{formTitle}</h2>
          <button type="button" class="text-xs text-slate-400 transition-colors hover:text-white" onclick={closeForm}>
            Cancel
          </button>
        </div>
        ...
        <div class="mt-4 grid gap-4 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
          <div class="space-y-4">
            ...
            <div class="flex flex-wrap items-center gap-2 pt-1">
              ...
              <Button type="button" variant="ghost" size="sm" class="rounded-xl text-slate-300 hover:text-white" onclick={closeForm} disabled={isSaving}>
                Cancel
              </Button>
            </div>
          </div>
          ...
          <div class="space-y-2">
            ...
            <textarea
              ...
              rows="10"
              class="flex min-h-[15rem] w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white placeholder:text-slate-500 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
              disabled={isSaving}
            ></textarea>
          </div>
        </div>
      ...
      {:else}
        <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {#each keys as key (key.id)}
            <article class="rounded-2xl border border-white/8 bg-white/[0.03] px-4 py-3">
              <div class="flex items-start gap-3">
                <div class="mt-0.5 flex size-10 shrink-0 items-center justify-center rounded-2xl bg-white/6 text-primary">
              ...
              <div class="mt-4 flex items-center gap-2">
                <Button variant="ghost" size="xs" class="gap-1.5 rounded-xl text-slate-300 hover:text-white" onclick={() => handleEdit(key)}>
                  <Pencil class="size-3" />
                  Edit
                </Button>
                <Button variant="ghost" size="xs" class="gap-1.5 rounded-xl text-slate-300 hover:text-destructive" onclick={() => handleDelete(key)}>
                  <Trash2 class="size-3" />
                  Delete
                </Button>
              </div>
